...
export async function requestJSON(url, opts = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    if (!res.ok) {
      // A provider that puts its real diagnosis in the body gets first say, even on a
      // non-2xx. Meta answers HTTP 400 for everything from a dead token (code 190) to a
      // throttle (code 4) to a too-heavy query (code 2), so the status alone would
      // flatten all of them into reason:"http". Every `accept` here is guard-first — it
      // returns when the body carries no error it recognises, and we fall through to
      // the status-based mapping below.
      if (accept && json) {
        try {
          accept(json, res);
        } catch (e) {
