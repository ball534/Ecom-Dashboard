...
// Orders FULFILLED (from the fulfillments dataset) — distinct from orders PLACED
// (`FROM sales SHOW orders`). An order placed late in a month may be fulfilled the
// next month, so the two figures differ; we surface both so the dashboard reconciles
// with Shopify's "Orders fulfilled" report as well as its "Orders" report.
export function buildFulfillmentsQL(start, end) {
  return `FROM fulfillments SHOW orders_fulfilled TIMESERIES month SINCE ${start} UNTIL ${end}`;
}
...
export const FULFILLMENTS_SPEC = {
  presence: "orders_fulfilled",
  fields: [{ col: "orders_fulfilled", metric: "ordf" }],
};
...
export function bucketFulfillments(rows, years) {
  return bucketShopifyQL(rows, { years, ...FULFILLMENTS_SPEC });
}
