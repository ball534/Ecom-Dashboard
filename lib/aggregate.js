...
export function normalizeOrder(node) {
  ...
  // Revenue: the CURRENT grand total — already net of refunds and order edits, and
  // (with SG tax-inclusive pricing) inclusive of GST + shipping. This is Shopify's
  // "total sales" figure and matches the admin Sales report.
  ...
  // Units = NET items sold: ordered quantity minus refunded/returned quantity.
  // (Shopify Analytics reports net items; counting gross over-states units — e.g.
  // Apr 2026 had 320 refunded units that must come out.)
  let ordered = 0;
  for (const e of node?.lineItems?.edges ?? []) {
    ordered += Number(e?.node?.quantity ?? 0) || 0;
  }
  let refunded = 0;
  for (const r of node?.refunds ?? []) {
    for (const e of r?.refundLineItems?.edges ?? []) {
      refunded += Number(e?.node?.quantity ?? 0) || 0;
    }
  }
  const units = Math.max(0, ordered - refunded);
