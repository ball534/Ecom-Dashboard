...
export function normalizeOrder(node) {
  ...
  // Units = GROSS quantity ordered — matches Shopify's "Items ordered over time"
  // report (quantity_ordered), which counts ordered items and does NOT subtract
  // returns/refunds (nor exclude cancelled orders). Revenue, by contrast, uses the
  // CURRENT total so it IS net of refunds — that mirrors Shopify's items-vs-sales split.
  let units = 0;
