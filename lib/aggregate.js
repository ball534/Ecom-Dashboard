...
// Is each order the customer's FIRST-ever purchase? Returns true | false | null
// (null = guest checkout), index-aligned with `orders`.
//
// Accurate without pulling full order history: the live window is the CURRENT calendar
// year, so any order a customer has outside this set must be a PRIOR-year one. If their
// lifetime count (customer.numberOfOrders) equals their count in-window, their first-ever
// order is here — the earliest one is "new", the rest "returning". A higher lifetime
// count means they pre-date the window, so every order here is "returning".
function newReturningFlags(orders) {
  ...
  return orders.map((o, i) => {
    if (o?.customerId == null || o?.numOrders == null) return null;
    const firstOrderIsInWindow = o.numOrders <= (liveCount.get(o.customerId) || 0);
    return Boolean(firstOrderIsInWindow && earliestIdx.get(o.customerId)?.i === i);
  });
}

// Same classification, materialised as `{...order, isNew}` records.
// bucketOrders uses newReturningFlags instead — at year scale this copied every order
// (and its line array reference) purely to carry one boolean.
export function classifyNewReturning(orders) {
  const flags = newReturningFlags(orders);
  return orders.map((o, i) => ({ ...o, isNew: flags[i] }));
}

// Formatters are cached per timezone: BUILDING an Intl.DateTimeFormat costs ~26x more
// than using one (50k calls measured at 2182ms vs 84ms), and a year-scale pull calls
// monthIndexInTZ once per order per bucketing pass.
const TZ_FORMATTERS = new Map();
function tzFormatter(timeZone) {
  let f = TZ_FORMATTERS.get(timeZone);
  if (!f) {
    f = new Intl.DateTimeFormat("en-US", { timeZone, year: "numeric", month: "numeric" });
    TZ_FORMATTERS.set(timeZone, f);
  }
  return f;
}

// Year + 0-based month of an ISO timestamp in an IANA timezone (the shop's), so orders
// near midnight land in the right month.
export function monthIndexInTZ(iso, timeZone = "Asia/Singapore") {
  const parts = tzFormatter(timeZone).formatToParts(new Date(iso));
  let year = null;
  let month = null;
  // One pass instead of two .find() scans — same values, no intermediate closures.
...
// "Today" as YYYY-MM-DD in a timezone (en-CA formats as ISO). Shared by every endpoint
// that defaults its window to the current year in the shop's timezone.
export function todayInTZ(timeZone = "Asia/Singapore") {
...
export function bucketOrders(orders, { years, timeZone = "Asia/Singapore" } = {}) {
  ...
  // New/returning needs the full set per customer, so resolve the flags up front.
  const isNewFlags = newReturningFlags(orders);
  ...
  orders.forEach((o, i) => {
    ...
    const isNew = isNewFlags[i];
    if (o.customerId != null && isNew != null) {
      (isNew ? newCust : retCust)[year][month].add(o.customerId);
    }
  });
